...
class PatchViewer(ipywidgets.VBox):
    ...
    """
    def __init__(self, images, boxes, patch, overlap=None, label=True, color=None, size=None, alpha=50, info=False, draw_box=None, width=800, height=500, **kwargs):
        self.images = images
        self.info = info
        self.clicked = None
    ...
    def draw(self, index):
        ...
        for img_idx, imgdata in enumerate(self._img_data):
            if idx >= imgdata[0]:
                idx -= imgdata[0]
            else:
                break
        ...
        # Image
        lbl = str(self.boxes.image.cat.categories[img_idx])
        ...
        if self._draw_box_max == 3:
            ...
            self.patch_all_boxes = self.patch_all_boxes[
                (self.patch_all_boxes.x_top_left.between(x0, x1) & self.patch_all_boxes.y_top_left.between(y0, y1)) | 
                ((self.patch_all_boxes.x_top_left + self.patch_all_boxes.width).between(x0, x1) & (self.patch_all_boxes.y_top_left + self.patch_all_boxes.height).between(y0, y1))
            ]
    ...
    def click_prev_img(self, btn):
        ...
        img_idx -= 1
        if img_idx < 0:
            img_idx = len(self._img_data) - 1

        self.inp_idx.value = sum(imgdata[0] for imgdata in self._img_data[:img_idx])
    ...
    def click_next_img(self, btn):
        ...
        img_idx += 1
        if img_idx >= len(self._img_data) - 1:
            img_idx = 0
